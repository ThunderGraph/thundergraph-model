...
End-to-end ThunderGraph showcase: **authoritative requirements first** in [`program/l1_requirement_packages.py`](./program/l1_requirement_packages.py) — **package-level** `parameter` / `attribute` / `constraint` on the mission closure package, plus a deliberate **advanced leaf reqcheck** pattern (`model.requirement`, `requirement_input`, **`requirement_attribute`**, `requirement_accept_expr`) for mission-envelope margins — then citations and `allocate` in [`program/cargo_jet_program.py`](./program/cargo_jet_program.py) **→** composition **→** roll-ups **→** external compute **→** citations **→** verification, with deliberate **`allocate`** traceability. The root `System` stays structural: scenario parameters, citations, allocations, and the composed `aircraft` part. After `instantiate`, derived requirement values appear on **`ConfiguredModel.requirement_value_slots`**.
